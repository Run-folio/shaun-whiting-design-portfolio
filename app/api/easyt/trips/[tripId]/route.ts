...
import { deleteTripForOwner, getTripForOwner, saveTripForOwner } from "@/lib/easyt/repository";
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (!isEasyTTrip(body) || body.id !== tripId) return NextResponse.json({ error: "Invalid EasyT trip document." }, { status: 400 });

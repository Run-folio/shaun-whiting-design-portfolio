...
const imageByRegion: Record<RouteRegion, string> = {
  ...
  return (
    <section className={styles.browser}>
      ...
      <div className={styles.grid}>
        {filtered.map((route) => <Link className={styles.card} href={`/journey/routes/${route.key}`} key={route.key}>
          <div className={styles.image} style={{ backgroundImage: `url(${imageByRegion[route.region]})` }}><span>{route.region.replace("-", " ")} · {route.suggestedDays.ideal} days</span></div>

...
export default function StampedClient({ userKey }: Props) {
  ...
  const filtered = continent === "All" ? countries : countries.filter((country) => country.continent === continent);
  ...
  const sortCountries = (items: Country[]) => [...items].sort((a, b) => {
    const aRank = statuses[a.id] === "visited" ? 0 : 1;
    const bRank = statuses[b.id] === "visited" ? 0 : 1;
